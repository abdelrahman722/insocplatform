<div class="relative mb-6 w-full">
    ...
    <flux:subheading size="lg" class="mb-6">
        <div class="grid grid-cols-2 gap-5 sm:grid-cols-2 lg:grid-cols-2">
            <div >
                {{ __('Manage your All Students') }}
            </div>
            <div class="text-right">
                <a href="{{ route('manager.students.create') }}" class="cursor-pointer px-3 py-2 text-xs font-medium text-white bg-green-700 rounded-lg hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                    Create Student
                </a>
            </div>
        </div>
    ...
    <div>
        <div class="overflow-x-auto p-3">
                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr
                                ...
                                <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $user->student->code }}</td>
                                <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $user->name }}</td>
                                <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $user->student?->phone }}</td>
                                <td class="px-6 py-2 text-gray-600 dark:text-gray-300">{{ $user->student?->semester?->name }}</td>
                                <td class="px-6 py-2">
                                    <flux:button href="{{ route('manager.students.edit', $user->id) }}" icon="pencil-square"
                                        size="xs" variant="primary" color="blue"></flux:button>

                                    <flux:modal.trigger name="delete-user-student-{{ $user->id }}">
                                        <flux:button icon="trash" size="xs" variant="danger" color="red" class="ml-1.5"></flux:button>
                                    </flux:modal.trigger>

                                    <flux:modal wire:key="delete-user-student-{{ $user->id }}" name="delete-user-student-{{ $user->id }}" class="min-w-[22rem]">
                                        <div class="space-y-6">
                                            <div>
            ...
            @else
                <flux:callout>
                    <flux:callout.heading icon="user-circle">Student</flux:callout.heading>
                    <flux:callout.text>
                        no student add yat.
